...
while node[2]!="Z" and step<length:
  ...
  if neighbour in path: # there is a match
    #print(f"neighbour {neighbour} is in path {path} ")
    #y = input("enter a key: ")
    
    # doing the usual move
    #dir = instructions[step]
    path.append((neighbour))
    nodesInPath = np.append(nodesInPath, (node))
    if neighbour not in matches: #first time finding the match
      ...
      for i in range(1,len(dir_now)):
        l = dir_now[i]
        r = instructions[step+1+i]
        if l == r:
          #this is worth cutting a branch even though there are only two next matches. 
          neighbour = path[i]
          path.append(neighbour)
          nodesInPath = np.append(nodesInPath, (nodesInPath[i]))
          print(f"checking out at {i} and {step+1}")
          print(f"neighbour {neighbour} and node {node}")
          y = input("enter a key: ")
        else:
          branch = []
      ...
      #working here
    else: #print(f"Yes, there are more than one matches at {neighbour} at {step} ")
      matches[neighbour].append(step)
    print(matches)
    y = input("enter a key: ")

  else: #neighbour not in path no match. so continue as usual
    dir = instructions[step]
    path.append((neighbour))
    nodesInPath = np.append(nodesInPath, (node))
    #print(f"path = {path}")
